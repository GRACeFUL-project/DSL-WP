...
The following example defines a component, which represents a pump with a 
given capacity:
\begin{haskellcode}
pump :: Int -> Component (Port Int, Param Int)
pump cap = do
  flow     <- createPort
  capacity <- createParam cap
  embed constraints
...
paramter (|cap|) and returns a GRACe component with a one port and one
parameter. This can be deducted from the type signature on first line in the
above code snippet. The port and parameter can be connected to other
components. A port is created with a call to the |createPort| function, and is
of a particular type, such as the integer port |flow| that we have defined
above. A GRACe parameter is created with the |createParam| function and can be
regarded as a port with an initial value. It is important to distinguish between
normal (Haskell) parameters, such as |cap|, and parameters defined in the GRACe
DSL, such as |capacity|. Parameters defined in the GRACe DSL can be connected to
other components, which may influence its value. 
...
its maximum capacity. Using the |constrain| function we can embed a number of
constraints in the component. In fact, the |constrain| function takes a 
representation of a constraint program as argument. A constraint program is The |assert| func. 
